...
fun createSetBlocksTool(fawe: FaweAdapter): ToolDefinition {
    return ToolDefinition(
        ...
        inputSchemaJson = """
            {
                ...
                "properties": {
                    ...
                    "material": {"type": "string", "description": "Block material (e.g., STONE, DIAMOND_BLOCK)"}
                },
        ...
        val result = fawe.setBlocks(pos1, pos2, material)
        if (result.isSuccess) {
            McpSchema.CallToolResult(
                listOf(McpSchema.TextContent("Set ${result.getOrNull()} blocks to $material")),
                false
            )
        } else {
            McpSchema.CallToolResult(
                listOf(McpSchema.TextContent("Failed to set blocks: ${result.exceptionOrNull()?.message}")),
                true
            )
        }
    }
